...
describe('POI enrichment orchestration', (): void => {
	...
	test('retries only photos from a version 1 empty-photo cache entry', async (): Promise<void> => {
		...
		).run(tripId, feature.id, cachedAt);
		db.prepare(
		...
		expect(
			(
				db
					.prepare(
						"SELECT schema_version FROM trip_poi_enrichment_cache WHERE trip_id = ? AND feature_id = ? AND provider = 'tripadvisor'"
					)
					.get(tripId, feature.id) as { schema_version: number }
			).schema_version
		).toBe(3);
	});
